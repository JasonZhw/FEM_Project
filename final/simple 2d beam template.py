...
for elem in elems:
    ...
    for gp, w in zip(gauss_points, weights):
        ...
        for i in range(3):
            plotdata[i, e * gauss_points.size()[0] + q, 2] = stress[i] #voigt[i, 0], voigt[i, 1]]
            plotdata_strain[i, e * gauss_points.size()[0] + q, 2] = strain[i]
        q += 1
    e += 1
stress_components = {
    (0, 0): "Stress o_X",
...
for i in range(3):
    ...
    plt.scatter(plotdata_strain[i, :, 0], plotdata_strain[i, :, 1], c=plotdata_strain[i, :, 2] / max_stre_val, s=20, cmap=mpl.cm.jet)  # , xgauss[1],  c=stre_val)
...
for i in range(3):
    ...
    plt.scatter(plotdata[i, :, 0], plotdata[i, :, 1], c=plotdata[i, :, 2] / max_stre_val, s=20, cmap=mpl.cm.jet)  # , xgauss[1],  c=stre_val)
    stress_name = stress_components[(voigt[i, 0].item(), voigt[i, 1].item())]
    plt.title(stress_name)
    plt.colorbar()
    plt.axis('equal')
