...
for node in inner_nodes:
    plt.scatter(x[node, 0].numpy(), x[node, 1].numpy(), c='yellow', marker='s', s=50, label='Dirichlet' if node == inner_nodes[0] else "")
...
strain_components = {
    (0, 0): "Strain in x direction",
    (1, 1): "Strain in y direction",
    (0, 1): "Shear Strain",
    (1, 0): "Shear Stress T_xy"
}
...
for i in range(3):
    ...
    print("最大的应力：", max_stre_val)
    plt.scatter(plotdata[i, :, 0], plotdata[i, :, 1], c=plotdata[i, :, 2] / max_stre_val, s=50, cmap=mpl.cm.jet)  # , xgauss[1],  c=stre_val)
    plt.colorbar()
    plt.axis('equal')
...
vonmises = torch.sqrt(plotdata[0,:,2]**2 + plotdata[1,:,2]**2 - plotdata[0,:,2]*plotdata[1,:,2] + 3*plotdata[2,:,2]**2)

plt.figure(figsize=(16, 16))
plt.scatter(plotdata[0,:,0], plotdata[0,:,1], c=vonmises, s=50, cmap=mpl.cm.jet)
...
x_coords = x[:, 0].numpy()
y_coords = x[:, 1].numpy()
stress_values = von_mises_node_stress.numpy()  # 使用 node_von_mises_stress 数据
