...
msh_file = 'D:\codes\gid\data_v4.msh' #disp_scaling=1e8 674 elements
...
gauss_positions = []
'''
node_stress_sum = torch.zeros((num_nodes, 3), dtype=torch.float64)
node_stress_count = torch.zeros(num_nodes, dtype=torch.float64)
...
plotdata_strain=torch.zeros(3, elems.size()[0] * gauss_points.size()[0], 3)
x_disped = x + disp_scaling * u_reshaped
...
for elem in elems:
    ...
    for gp, w in zip(gauss_points, weights):
        ...
        for i in range(3):
            ...
            plotdata_strain[i, e * gauss_points.size()[0] + q, 0:2] = xgauss
        ...
        for i in range(3):
            ...
            plotdata_strain[i, e * gauss_points.size()[0] + q, 2] = eps[voigt[i, 0], voigt[i, 1]]
        N = shape_functions(xi, eta)
        for i in range(nen):
            node_stress_sum[elem[i]] += sigma[[0, 1, 0], [0, 1, 1]]
            node_stress_count[elem[i]] += 1
        q += 1
    e += 1
...
for i in range(3):
    ...
    sc = ax.scatter(plotdata[i, :, 0], plotdata[i, :, 1], c=plotdata[i, :, 2] / max_stre_val, s=50, cmap='jet')
    ...
    for elem in elems:
        elem = elem.numpy()  # Convert to numpy for plotting
        ax.plot(x_disped[elem[[0, 1, 2, 3, 0]], 0].numpy(), x_disped[elem[[0, 1, 2, 3, 0]], 1].numpy(), 'black', linewidth=0.5)
    ax.scatter(x_disped[:, 0].numpy(), x_disped[:, 1].numpy(), c='black', s=1, zorder=5)

# 确保 von_mises_gauss_stress 的长度与 plotdata[i, :, 0] 和 plotdata[i, :, 1] 的长度一致
von_mises_gauss_stress = torch.sqrt(plotdata[0, :, 2]**2 + plotdata[1, :, 2]**2 - plotdata[0, :, 2] * plotdata[1, :, 2] + 3 * plotdata[2, :, 2]**2)
max_stre_val = torch.max(torch.abs(von_mises_gauss_stress)) + 1e-12
print("最大的等效应力：", max_stre_val)
# 绘制Von Mises应力图
ax = axs[1, 1]
sc4 = ax.scatter(plotdata[0, :, 0], plotdata[0, :, 1], c=von_mises_gauss_stress/max_stre_val.numpy(), cmap='jet', s=50)
fig.colorbar(sc4, ax=ax)
ax.axis('equal')
ax.set_title('Von Mises Stress (Gauss Points)')
...
strain_components = {
    (0, 0): "Strain o X",
    (1, 1): "Strain o_y",
    (0, 1): "Shear Strain T_xy",
    (1, 0): "Shear Strain T_xy"
}
plt.figure(figsize=(24,8))
for i in range(3):
    plt.subplot(2,2, i+1)
    max_stre_val = torch.max(torch.abs(plotdata_strain[i, :, 2])) + 1e-12
    if i == 2:
        print("最大的剪切应变：", max_stre_val)
    plt.scatter(plotdata_strain[i, :, 0], plotdata_strain[i, :, 1], c=plotdata_strain[i, :, 2] / max_stre_val, s=50, cmap=mpl.cm.jet)  # , xgauss[1],  c=stre_val)
    plt.colorbar()
    plt.axis('equal')
    #plt.title ("sigma_" + str(voigt[i, 0].item()) + str(voigt[i, 1].item()))
    strain_name = strain_components[(voigt[i, 0].item(), voigt[i, 1].item())]
    plt.title(strain_name)
plt.tight_layout()
plt.show()
print(torch.max(u))
# 计算每个节点的平均应力
node_stress_avg = node_stress_sum / node_stress_count.view(-1, 1)

# 绘制节点应力分布图
fig, axs = plt.subplots(2, 2, figsize=(16, 16))

# Stress σ_x
sc1 = axs[0, 0].scatter(x_disped[:, 0].numpy(), x_disped[:, 1].numpy(), c=node_stress_avg[:, 0].numpy(), cmap='jet', s=50)
#axs[0, 0].scatter(x_disped[:, 0].numpy(), x_disped[:, 1].numpy(), c='black', s=1, zorder=5)
axs[0, 0].set_title('Node Stress σ_x')
axs[0, 0].axis('equal')
fig.colorbar(sc1, ax=axs[0, 0])

# Stress σ_y
sc2 = axs[0, 1].scatter(x_disped[:, 0].numpy(), x_disped[:, 1].numpy(), c=node_stress_avg[:, 1].numpy(), cmap='jet', s=50)
#axs[0, 1].scatter(x_disped[:, 0].numpy(), x_disped[:, 1].numpy(), c='black', s=1, zorder=5)
axs[0, 1].set_title('Node Stress σ_y')
axs[0, 1].axis('equal')
fig.colorbar(sc2, ax=axs[0, 1])

# Shear Stress τ_xy
sc3 = axs[1, 0].scatter(x_disped[:, 0].numpy(), x_disped[:, 1].numpy(), c=node_stress_avg[:, 2].numpy(), cmap='jet', s=50)
#axs[1, 0].scatter(x_disped[:, 0].numpy(), x_disped[:, 1].numpy(), c='black', s=1, zorder=5)
axs[1, 0].set_title('Node Shear Stress τ_xy')
axs[1, 0].axis('equal')
fig.colorbar(sc3, ax=axs[1, 0])

# Von Mises Stress
von_mises_node_stress = torch.sqrt(node_stress_avg[:, 0]**2 + node_stress_avg[:, 1]**2 - node_stress_avg[:, 0] * node_stress_avg[:, 1] + 3 * node_stress_avg[:, 2]**2)
sc4 = axs[1, 1].scatter(x_disped[:, 0].numpy(), x_disped[:, 1].numpy(), c=von_mises_node_stress.numpy(), cmap='jet', s=50)
#axs[1, 1].scatter(x_disped[:, 0].numpy(), x_disped[:, 1].numpy(), c='black', s=1, zorder=5)
axs[1, 1].set_title('Node Von Mises Stress')
axs[1, 1].axis('equal')
...
# 确保 von_mises_gauss_stress 的长度与 plotdata[i, :, 0] 和 plotdata[i, :, 1] 的长度一致
von_mises_gauss_stress = torch.sqrt(plotdata[0, :, 2]**2 + plotdata[1, :, 2]**2 - plotdata[0, :, 2] * plotdata[1, :, 2] + 3 * plotdata[2, :, 2]**2).numpy()

# 提取高斯点的坐标
gauss_x = plotdata[0, :, 0].numpy()
gauss_y = plotdata[0, :, 1].numpy()

# 创建更密集的网格
grid_x, grid_y = np.mgrid[gauss_x.min():gauss_x.max():1000j, gauss_y.min():gauss_y.max():1000j]

# 使用插值函数在密集网格上进行插值
grid_z = griddata((gauss_x, gauss_y), von_mises_gauss_stress, (grid_x, grid_y), method='cubic')
...
plt.imshow(grid_z.T, extent=(gauss_x.min(), gauss_x.max(), gauss_y.min(), gauss_y.max()), origin='lower', cmap='jet')
plt.colorbar(label='Von Mises Stress')
plt.title('Von Mises Stress Distribution')
plt.xlabel('X')
plt.ylabel('Y')
plt.scatter(gauss_x, gauss_y, c='black', s=5, zorder=5)
plt.axis('equal')
plt.show()