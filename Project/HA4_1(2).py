...
def huber_stress(stress):
    return von_mises_stress(stress)  # Huber stress is the same as von Mises stress

def hencky_stress(stress):
    return von_mises_stress(stress)  # Hencky stress is the same as von Mises stress
...
num_elements_radial = 8
num_elements_circumferential = 16
...
plt.scatter(x[highlight_node, 0].numpy(), x[highlight_node, 1].numpy(), c='red', s=50, zorder=10)  # 高亮显示选定的点
...
for node in bottom_nodes:
    neum[node, 1] += N_a / len(bottom_nodes)
    neum[node, 0] -= F_a / len(bottom_nodes)
...
for node in side_nodes_left:
    neum[node, 1] += F_b / len(side_nodes_left)
    neum[node, 0] += N_b / len(side_nodes_left)
for node in side_nodes_right:
    neum[node, 1] -= F_b / len(side_nodes_right)
    neum[node, 0] -= N_b / len(side_nodes_right)
'''
# Apply N_a and F_a at the bottom node
bottom_nodes = torch.where(torch.abs(x[:, 1] - torch.min(x[:, 1])) <= node_tolerance)[0]
for node in bottom_nodes:
...
# 标记Neumann边界条件节点
#for node in bottom_nodes:
    #plt.scatter(x[node, 0].numpy(), x[node, 1].numpy(), c='red', marker='^', s=100, label='Neumann' if node == bottom_nodes[0] else "")
#for node in side_nodes_left:
    #plt.scatter(x[node, 0].numpy(), x[node, 1].numpy(), c='red', marker='^', s=100, label='Neumann' if node == side_nodes_left[0] else "")
#for node in side_nodes_right:
    #plt.scatter(x[node.item(), 0].numpy(), x[node.item(), 1].numpy(), c='red', marker='^', s=100, label='Neumann' if node == side_nodes_right[0] else "")
...
# 绘制所有应力分量
    ...
    huber_stress_all_nodes = torch.tensor([huber_stress(s) for s in stress_all_nodes])
    hencky_stress_all_nodes = torch.tensor([hencky_stress(s) for s in stress_all_nodes])

    fig, axs = plt.subplots(1, 3, figsize=(20, 5))

    sc1 = axs[0].scatter(x_disped[:, 0].numpy(), x_disped[:, 1].numpy(), c=von_mises_stress_all_nodes.numpy(), cmap='jet', s=5)
    axs[0].set_title('Von Mises Stress')
    axs[0].axis('equal')
    fig.colorbar(sc1, ax=axs[0])

    sc2 = axs[1].scatter(x_disped[:, 0].numpy(), x_disped[:, 1].numpy(), c=huber_stress_all_nodes.numpy(), cmap='jet', s=5)
    axs[1].set_title('Huber Stress')
    axs[1].axis('equal')
    fig.colorbar(sc2, ax=axs[1])

    sc3 = axs[2].scatter(x_disped[:, 0].numpy(), x_disped[:, 1].numpy(), c=hencky_stress_all_nodes.numpy(), cmap='jet', s=5)
    axs[2].set_title('Hencky Stress')
    axs[2].axis('equal')
    fig.colorbar(sc3, ax=axs[2])
...
plt.scatter(x_disped[highlight_node, 0].numpy(), x_disped[highlight_node, 1].numpy(), c='red', s=50, zorder=10)  # 高亮显示选定的点
